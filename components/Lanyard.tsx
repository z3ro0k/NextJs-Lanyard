...
const Lanyard: FC<Props> = ({ id }) => {
  ...
  return (
    <Doing>
      <Flex>
        <AlbumArt>
          <ActivityImage src={doing.spotify.album_art_url} width={69} height={69} />
          <ActivitySecondaryImage />
        </AlbumArt>
        <Details>
          <Song>{doing.spotify.song}</Song>
          <Artist>{doing.spotify.artist.split(";").join(", ")}</Artist>
        </Details>
        <Live />
...
const ActivitySecondaryImage = styled(Spotify)`
